/*
TV-Shut-Up Firmware version 1.2
...
/*
This project transmits a bunch of TV MUTE codes, one right after the other,
with a pause in between each.  (To have a visible indication that it is
transmitting, it also pulses a visible LED once each time a MUTE code is
transmitted.)  That is all TV-Shut-Up does.  The tricky part of TV-Shut-Up
was collecting all of the MUTE codes, and getting rid of the duplicates and
near-duplicates (because if there is a duplicate, then one MUTE code will
mute a TV , and the duplicate will unmute it on again (which we certainly
do not want).

Before creating a TV-B-Gone Kit, they originally started this project by hacking
...
fit about 7 MUTE codes into the firmware's database of MUTE codes.  However,
the more codes the better! Which is why they chose the ATtiny85 for the
TV-B-Gone Kit.
*/
...
extern const PGM_P * const NAmuteCodes[] PROGMEM;
extern const PGM_P * const EUmuteCodes[] PROGMEM;
...
the microcontroller resets.  Since this firmware has a table (muteCodes)
...
(this is used in the definition for muteCodes).  Since the C compiler assumes
that constants are in RAM, rather than in program memory, when accessing
muteCodes, we need to use the pgm_read_word() and pgm_read_byte macros, and
we need to use muteCodes as an address.  This is done with PGM_P, defined
below.
For example, when we start a new muteCode, we first point to it with the
following statement:
    PGM_P thecode_p = pgm_read_word(muteCodes+i);
The next read from the muteCode is a byte that indicates the carrier
...
Subsequent reads from the muteCode are n bits (same as the packing size)
...
int main(void) {
  ...
  do {	//Execute the code at least once.  If Loop is on, execute forever.
    ...
    // for every MUTE code in our collection
    for(i=0 ; i < j; i++) {
      ...
      // point to next MUTE code, from the right database
      if (region == US) {
	code_ptr = (PGM_P)pgm_read_word(NAmuteCodes+i);
      } else {
	code_ptr = (PGM_P)pgm_read_word(EUmuteCodes+i);
      }

      // print out the address in ROM memory we're reading
      DEBUGP(putstring("\n\rAddr: "); putnum_uh((int)code_ptr));

      // Read the carrier frequency from the first byte of code structure
      const uint8_t freq = pgm_read_byte(code_ptr++);
      // set OCR for Timer1 to output this MUTE code's carrier frequency
      OCR0A = freq;
      ...
      // Transmit all codeElements for this MUTE code
	...
	xmitCodeElement(ontime, offtime, (freq!=0));
      ...
      // delay 250 milliseconds before transmitting next MUTE code
      delay_ten_us(25000);
